...
def get_display_by_id(display_id):
    ...
    return dict(
        ...
        label=label,
        plot_type=plot_type,
        plotly_config=json.loads(plotly_config),
    )
...
class Layout:
    ...
    def add_member(self, member):
        ...
        if member is None:
            raise Exception(
                "ERROR: Attempted to add a None member to Layout {0}".format(self.id)
            )
    ...
    def get_members(self, scope="all"):
        ...
        if scope not in ["all", "single", "multi"]:
            raise Exception(
                "ERROR: Invalid scope '{0}' passed to Layout.get_members()".format(
                    scope
                )
            )
...
class LayoutCollection:
    ...
    def get_by_share_id(self, share_id=None):
        ...
        for row in rows:
            ...
            if self.include_datasets:
                layout.get_members()
                layout.dataset_count = len(
                    layout.members or []
                )  # Excludes datasets marked for removal
    ...
    def get_by_user(self, user=None):
        ...
        if not isinstance(user, User):
            raise Exception(
                "LayoutCollection.get_by_user() requires an instance of User to be passed."
            )
    ...
    def get_by_users_groups(self, user=None, append=True):
        ...
        if not isinstance(user, User):
            raise Exception(
                "LayoutCollection.get_by_users_groups() requires an instance of User to be passed."
            )
...
class FolderCollection:
    ...
    def get_root_folders(self, folder_type=None):
        ...
        for scope in ["domain", "user", "group", "shared", "public"]:
            ...
            if folder_id in rfs:
                folder = Folder(id=folder_id, parent_id=None, label=rfs[folder_id])
                self.folders.append(folder)
    ...
    def get_tree_by_folder_ids(self, ids=None, folder_type=None):
        ...
        while len(new_ids_found) > 0:
            ...
            for row in rows:
                folder = Folder(
                    id=row["id"], parent_id=row["parent_id"], label=row["label"]
                )
...
class Dataset:
    ...
    def get_file_path(self, session_id=None):
        ...
        if self.dtype == "spatial":
            if session_id is None:
                ...
                zarr_file_path = "{0}/{1}/spatial/{2}.zarr".format(
                    this.analysis_base_dir, session_id, self.id
                )
        ...
        if session_id is None:
            ...
            h5ad_file_path = "{0}/{1}/{2}.h5ad".format(
                this.analysis_base_dir, session_id, self.id
            )
    ...
    def get_layouts(self, user=None, include_public=False):
        ...
        if len(self.layouts) < 1:
            ...
            self.layouts = list(
                {layout_obj.id: layout_obj for layout_obj in self.layouts}.values()
            )
    ...
    def get_links(self):
        ...
        if len(self.links) < 1:
            ...
            for id, resource, label, url in rows:
                dsl = DatasetLink(
                    dataset_id=self.id, resource=resource, label=label, url=url
                )
...
class DatasetCollection:
    ...
    def get_public(self, has_h5ad=None, n=None, order_by=None, types=None):
        ...
        for r in rows:
            dataset = Dataset(
                ...
                dtype=r["dtype"],
                schematic_image=r["schematic_image"],
                share_id=r["share_id"],
                math_default=r["math_default"],
                marked_for_removal=r["marked_for_removal"],
    ...
    def get_owned_by_user(self, has_h5ad=None, user=None, types=None):
        ...
        if user is None:
            raise Exception(
                "Error: get_owned_by_user() requires a User object to be passed."
            )
    ...
    def get_shared_with_user(self, has_h5ad=None, user=None, types=None):
        ...
        if user is None:
            raise Exception(
                "Error: get_shared_with_user() requires a User object to be passed."
            )
...
class Gene:
    ...
    def load_dbxrefs(self):
        ...
        for (dbxref,) in rows:
            ...
            self.dbxrefs.append(
                {"source": source, "identifier": identifier, "url": None}
            )
    ...
    def load_dbxref_links(self):
        ...
        self.dbxrefs.append(
            {
                "source": "PubMed",
                "identifier": self.gene_symbol,
                "url": "http://www.ncbi.nlm.nih.gov/pubmed/?term={0}".format(
                    self.gene_symbol
                ),
            }
        )
        ...
        if "HomoloGene" in this.links_out and this.links_out["HomoloGene"] is True:
            if self.gene_symbol is not None:
                hg_url = (
                    "https://www.ncbi.nlm.nih.gov/datasets/gene/taxon/1/?search="
                    + self.gene_symbol.upper()
                )
                self.dbxrefs.append(
                    {
                        "source": "HomoloGene",
        ...
        if this.domain_label == "gear":
            ...
            self.dbxrefs.append(
                {"source": "SHIELD", "identifier": self.gene_symbol, "url": None}
            )
            ...
            if "DVD" in this.links_out and this.links_out["DVD"] is True:
                ...
                if self.gene_symbol and self.gene_symbol.upper() in dvd_genes:
                    dvd_url = (
                        "http://deafnessvariationdatabase.org/gene/"
                        + self.gene_symbol.upper()
                    )
                    self.dbxrefs.append(
                        {
                            ...
                            "identifier": self.gene_symbol,
                            "url": dvd_url,
                            "title": "Deafness Variation Database",
                        }
                    )
        ...
        for dbxref in self.dbxrefs:
            ...
            if source == "CCDS":
                dbxref["url"] = (
                    "https://www.ncbi.nlm.nih.gov/CCDS/CcdsBrowse.cgi?REQUEST=CCDS&GO=MainBrowse&DATA={0}".format(
                        dbxref["identifier"]
                    )
                )
            elif source == "SHIELD":
                dbxref["url"] = (
                    "https://shield.hms.harvard.edu/viewgene.html?gene={0}".format(
                        dbxref["identifier"]
                    )
                )
            elif source == "UCSC":
                if self.organism_id == 1:
                    dbxref["url"] = (
                        "http://genome.ucsc.edu/cgi-bin/hgTracks?org=mouse&db=mm10&"
                        "singleSearch=knownCanonical&position={0}".format(
                            dbxref["identifier"]
                        )
                    )
            elif source == "UniParc":
                dbxref["url"] = "https://www.uniprot.org/uniparc/{0}?sort=score".format(
                    dbxref["identifier"]
                )
            elif source == "Uniprot/SWISSPROT":
                dbxref["url"] = "https://www.uniprot.org/uniprot/{0}".format(
                    dbxref["identifier"]
                )

                # Bonus, all swissprot entries are also indexed in Phosphosite
                phospho_url = (
                    "https://www.phosphosite.org/uniprotAccAction?id={0}".format(
                        dbxref["identifier"]
                    )
                )
                self.dbxrefs.append(
...
class GeneCollection:
    ...
    def get_by_gene_symbol(
        self, gene_symbol=None, exact=None, organism_id=None
    ):
        """
        ...
        for gene_symbol in gene_symbols:
            ...
            qry_select = """
              SELECT id, ensembl_id, ensembl_version, ensembl_release, genbank_acc, organism_id,
                ...
                FROM gene
            """
            qry_order = "ORDER BY gene_symbol, organism_id, ensembl_release DESC"


            # this comes from javascript as a string true/false
            if exact in [True, "true", 1, "1"]:
                qry_where = "WHERE gene_symbol = %s"
                qry_param = (gene_symbol,)
            else:
                qry_where = "WHERE gene_symbol LIKE %s"
                qry_param = ("%" + gene_symbol + "%",)

            qry = qry_select + qry_where + " " + org_addendum + qry_order

            cursor.execute(qry, qry_param)
            ...
            for (
                ...
                if organism_id in found[lc_gene_symbol]:
                    if (
                        found[lc_gene_symbol][organism_id]["ensembl_release"]
                        == ensembl_release
                    ):
                    ...
                    found[lc_gene_symbol][organism_id] = {
                        "ensembl_release": ensembl_release,
                        "count": 1,
                    }
    ...
    def update_from_json(self, json_obj):
        ...
        if "session_id" in json_obj and not self.user_id:
            ...
            if user_logged_in is None:
                raise Exception(
                    "Error: No user logged in with session_id {0}".format(
                        json_obj["session_id"]
                    )
                )
...
class LayoutMember:
    ...
    def save(self, layout=None):
        ...
        if layout is None:
            raise Exception(
                "You must pass a layout member object to save a layout member"
            )
