...
Processing file: /tmp/human/Homo_sapiens.GRCh38.93.chromosome.6.dat.gz
Gene count     : 2869.0
GO per gene    : 3.4
Dbxref per gene: 11.4

# After fixing indentation issue
Processing file: /tmp/human/Homo_sapiens.GRCh38.93.chromosome.6.dat.gz
Gene count     : 2869
GO per gene    : 3.4
Dbxref per gene: 11.4

# Now taking first CDS per gene rather than last
Processing file: /tmp/human/Homo_sapiens.GRCh38.93.chromosome.6.dat.gz
Gene count     : 2869
GO per gene    : 4.9
Dbxref per gene: 15.3

Processing file: /tmp/human/Homo_sapiens.GRCh38.93.chromosome.6.dat.gz
Gene count     : 2869
GO per gene    : 6.1
Dbxref per gene: 18.3
...
def main():
    ...
    parser.add_argument('-i', '--input_dir', type=str, required=True, help='Path to an input file to be read' )
    ...
    for file in sorted(files):
        if file.endswith('.dat.gz'):
            ...
            print("Processing file: {0}".format(fpath), file=sys.stderr)
    ...
    print("Gene count     : {0}".format(gene_count))
    print("GO per gene    : {0:.1f}".format(go_count / gene_count))
    print("Dbxref per gene: {0:.1f}".format(dbxref_count / gene_count))
...
def process_gb_record(record):
    ...
    for feat in record.features:
        ...
        if feat.type == 'gene':
            ...
            go_annotations = get_go_annotations(feat)
            dbxrefs = get_dbxrefs(feat)

            gene_score = len(go_annotations) + len(dbxrefs)
            
            if gene_score > gene_max_score:
                current_annot.other_attributes['genbank_acc'] = get_genbank_acc(feat)
                current_annot.go_annotations = go_annotations
                current_annot.dbxrefs = dbxrefs
                gene_max_score = gene_score
...
def process_gb_gene(gene):
    ...
    if m:
        ...
        raise Exception("Found an ensembl id ({0}) without a .version".format(annot.other_attributes['ensembl_version']))
